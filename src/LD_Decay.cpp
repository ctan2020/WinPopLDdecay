...
QString runPopLDdecay(const QString &vcfFile,
                      ...
                      const QString &ehh,
                      int outType) {
    std::ostringstream log; // 用于收集日志和结果

    // 使用智能指针管理参数结构体，避免手工 new/delete 带来的泄漏风险
    std::unique_ptr<In3str1v> paraFA04(new In3str1v);
    std::unique_ptr<Para_18> para_18(new Para_18);

    // 设置参数（由 GUI 传入）
    paraFA04->InStr1 = vcfFile.toStdString();
    paraFA04->InStr2 = outPrefix.toStdString();
    paraFA04->InInt = (maxDistKb > 0 ? maxDistKb : 300);
    paraFA04->Method = 2;
    paraFA04->TF = (outType >= 1 && outType <= 8) ? outType : 1;
    para_18->MAF = maf;
    para_18->Het = het;
    para_18->Miss = miss;
    if (!subPopFile.trimmed().isEmpty()) {
        paraFA04->SubPop = subPopFile.toStdString();
    }
    if (!ehh.trimmed().isEmpty()) {
        paraFA04->EHH = ehh.toStdString();
    }
    ...
    if ((!OUTTest.good())) {
        log << "open OUT File error: " << Stat << std::endl;
    ...
    if (!(paraFA04->InStr1).empty()) {
        ...
        if  (PhaseVCF) {
            if ((paraFA04->SubPop).empty()) {
                Read_VCF_IN_Phase(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
            } else {
                Read_SubPopVCF_IN_Phase(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
            }
        } else {
            if ((paraFA04->SubPop).empty()) {
                Read_VCF_IN(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
            } else {
                Read_SubPopVCF_IN(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
            }
        }
    }
    ...
    if (!(paraFA04->InStr3).empty()) {
        if  ((paraFA04->SubPop).empty()) {
            Read_Genotype_IN(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
        } else {
            Read_SubPopGenotype_IN(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
        }
    }

    if (!(paraFA04->EHH).empty()) {
        EHH_Region_LDDecay(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
        log << "EHH Region LD Decay finished." << std::endl;
        return QString::fromStdString(log.str());
    }

    // PairWise Compare
    if (((paraFA04->TF) == 4) || ((paraFA04->TF) == 5)) {
        PairWiseComNewOUT_A(paraFA04.get(), para_18.get(), SNPList, Flag_for_pro);
    } else {
        // 使用 vector 做连续内存预分配，便于 CPU cache 和潜在向量化
        std::vector<StarRsult> allStat(static_cast<size_t>((paraFA04->InInt) + 1));
        StarRsult *All_Stat = allStat.data();
        if ((paraFA04->Method) == 2) {
            PairWiseComV2(paraFA04.get(), para_18.get(), SNPList, All_Stat, Flag_for_pro);
        } else {
            PairWiseComV1(paraFA04.get(), para_18.get(), SNPList, All_Stat, Flag_for_pro);
        }
        OUTStatFile(paraFA04.get(), para_18.get(), All_Stat);
    }

    log << "Used [perl  ../bin/Plot_XX.pl ] to Plot the LDdecay" << std::endl;
